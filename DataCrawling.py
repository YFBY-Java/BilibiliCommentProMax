...
def download_all_comments_serial(Cookie, videoList, UserName):
    """
    串行下载所有视频的评论
    """
    results = []
    for i, bv in enumerate(videoList, 1):
        print(f"正在下载第 {i}/{len(videoList)} 个视频的评论: {bv}")
        try:
            result = download_comment(Cookie, bv, UserName)
...
if __name__ == '__main__':
    ...
    UserName = getUserName.get_UserName(mid,Cookie)
    # UserName = "崩坏星穹铁道"
    if UserName is None:
        # 结束程序
        exit()
    ...
    with open(f"资源列表/视频列表/{UserName}/{file_name}", mode='w', encoding='gbk', newline='', errors='ignore') as f:
        ...
        page = Page.get_VideoPage(mid,Cookie)  #
        counter = 1
        while True:
            if counter > page:
                print("视频列表获取完毕")
                break
            get_VideoList(mid, counter, csv_writer,Cookie)
            counter = counter + 1
            # time.sleep(1)
    ...
    download_all_comments_serial(Cookie, videoList, UserName)
